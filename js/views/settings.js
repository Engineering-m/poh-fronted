...
App.views.settings = {
  ...
  changePw: function() {
    ...
    document.getElementById('btn-pw').onclick = async function() {
      ...
      if(!o||!n){toast('Field wajib.','error');return;} if(n!==c){toast('Tidak cocok.','error');return;}
      try{await API.call('changePassword',App.token,{old_password:o,new_password:n});Modal.close();toast('Diubah.','success');}catch(e){toast(e.message,'error');}
    };
  },
  saveCfg: async function(k,v){try{await API.call('setIntegrationConfig',App.token,{key:k,value:v});toast('Disimpan.','success');}catch(e){toast(e.message,'error');}},
  backup: async function(){toast('Backup…','info');try{var r=await API.call('dailyBackup',App.token);toast('OK: '+r.name,'success');}catch(e){toast(e.message,'error');}}
};
